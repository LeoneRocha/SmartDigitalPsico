...
export class AddEditApplicationConfigSettingComponent implements OnInit {
    registerId: number;
    ...
    addRegister() {
        ...
        this.registerService.add(this.registerModel).subscribe({
            next: (response: ServiceResponse<ApplicationConfigSettingModel>) => { this.processAddRegister(response); }, error: (err) => { this.processAddRegisterErro(err); },
        });
    }
    updateRegister() {
        this.getValuesForm();
        this.registerService.update(this.registerModel).subscribe({
            next: (response: ServiceResponse<ApplicationConfigSettingModel>) => { this.processUpdateRegister(response); }, error: (err) => { this.processUpdateRegisterErro(err); },
        });
    }
    processAddRegister(response: ServiceResponse<ApplicationConfigSettingModel>) {
        CaptureTologFunc('processAddRegister-ApplicationConfigSetting', response);
    ...
    processUpdateRegister(response: ServiceResponse<ApplicationConfigSettingModel>) {
        CaptureTologFunc('processUpdateRegister-ApplicationConfigSetting', response);
        this.serviceResponse = response;
        this.modalSuccessAlert();
    }
    ...
    processLoadRegister(response: ServiceResponse<ApplicationConfigSettingModel>) {
        CaptureTologFunc('processLoadRegister-ApplicationConfigSetting', response);
        this.serviceResponse = response;
        this.fillFieldsForm();
    ...
    fillFieldsForm(): void {
        ...
        this.registerModel = {
            ...
            endPointUrl_Cache: responseData?.endPointUrl_Cache,
            endPointUrl_StorageFiles: responseData?.endPointUrl_StorageFiles,
        };
    ...
    modalErroAlert(msgErro: string, response: ServiceResponse<ApplicationConfigSettingModel>) {
        swal.fire({
            title: msgErro,
